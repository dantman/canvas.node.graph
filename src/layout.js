...
Layout.types.circle.prototype = CanvasNode.util.create(Layout.prototype);
(function(proto) {
	...
	proto.iterate = function() {
		...
		for ( var i = 1; i <= circles.length; i++ ) {
			...
			var r = this.radius * Math.sin(Math.PI/2 * this.i / this.n);
...
Layout.types.spring.prototype = CanvasNode.util.create(Layout.prototype);
(function(proto) {
	...
	proto._attract = function(n1, n2, k, length) {
		...
		var f = (Math.pow(d.d, 2) - Math.pow(this.k, 2)) / this.k * length;
        f = f * k * 0.5 + 1;
        f = f / d.d;
